...
{ # main
	...
	if( $command eq "GETVAL"){
	    if( $val =~ /^instance\/[0-9a-z-]*\/virt_cpu_total/ ){
		@vals = split(/\//, $val);
#		$val = $vals[0] . "-" . $vals[1] . "/libvirt/" . $vals[2]	
		$val = $vals[1] . "/libvirt/" . $vals[2]	
	    }
	    elsif($val =~ /^instance\/[0-9a-z-]*\/disk\// and $val_type =~ /^OPS/){
		@vals = split(/\//, $val);
#		    $val = $vals[0] . "-" . $vals[1] . "/libvirt/" . $vals[2] . "_ops-" .$vals[3]		    
		    $val = $vals[1] . "/libvirt/" . $vals[2] . "_ops-" .$vals[3]		    
	    
	    }
	    elsif($val =~ /^instance\/[0-9a-z-]*\/disk\// and $val_type =~ /^OCT/){
		@vals = split(/\//, $val);
#		    $val = $vals[0] . "-" . $vals[1] . "/libvirt/" . $vals[2] . "_octets-" .$vals[3]
		    $val = $vals[1] . "/libvirt/" . $vals[2] . "_octets-" .$vals[3]
	    }
	    elsif($val =~ /^instance\/[0-9a-z-]*\/if\// and $val_type =~ /^NET-PACKETS/){

#		print $date;
		@vals = split(/\//, $val);
#		    $val = $vals[0] . "-" . $vals[1] . "/libvirt/" . $vals[2] . "_packets-" .$vals[3]."-".$vals[4]
		    $val = $vals[1] . "/libvirt/" . $vals[2] . "_packets-" .$vals[3]
#		    ."-".$date
		    #."-".$vals[4]
	    }
	    elsif($val =~ /^instance\/[0-9a-z-]*\/if\// and $val_type =~ /^NET-OCTETS/){
		@vals = split(/\//, $val);
...
#	    print $line[0] . "\n";
	    ...
	    if( $line[0] =~ /^[0-9a-z-]*\/libvirt\/virt_cpu_total/ ){
                print "$vals->{$key}\n";
	    }
	    elsif($line[0] =~ /^[0-9a-z-]*\/libvirt\/disk_ops/){
	    ...
	    elsif($line[0] =~ /^[0-9a-z-]*\/libvirt\/disk_octets/){
	    ...
	    elsif($line[0] =~ /^[0-9a-z-]*\/libvirt\/if_packets/){
...
#		print "DEBUG: if_packets options ..." . $/;
	    ...
	    elsif($line[0] =~ /^[0-9a-z-]*\/libvirt\/if_octets/){
